...
import {
  ...
  MdLocalShipping, MdStar, MdPhone,
} from "react-icons/md";
...
// ─── Edit Modal (same as Users.jsx but self-contained) ────────────────────────
      ...
      >
        ...
        <div className="emodal-photo-section">
          <div className="emodal-photo-wrap">
            <img src={preview || "https://via.placeholder.com/150?text=User"} alt="avatar" className="emodal-avatar" />
            <label htmlFor="editPhoto2" className="emodal-photo-btn"><MdPhotoCamera /></label>
        ...
        <div className="emodal-body">
          <div className="eform-group">
            ...
            <input name="name" type="text" placeholder="Enter full name" value={form.name} onChange={handleChange} className={errors.name ? "input-error" : ""} />
          ...
          <div className="eform-group">
            ...
            <input name="password" type="password" placeholder="Leave blank to keep current" value={form.password} onChange={handleChange} className={errors.password ? "input-error" : ""} />
            {errors.password && <small className="ferror">{errors.password}</small>}
          </div>
          <div className="eform-group">
            <label>Confirm Password</label>
            <input name="confirmPassword" type="password" placeholder="Repeat new password" value={form.confirmPassword} onChange={handleChange} className={errors.confirmPassword ? "input-error" : ""} />
        ...
        >
          ...
          <div className="ud-hero-right">
            <div className="ud-contact-item">
              <MdEmail className="ud-ci-icon" />
              <span>{user.email}</span>
            </div>
            {user.phone && (
              <div className="ud-contact-item">
                <MdPhone className="ud-ci-icon" />
                <span>{user.phone}</span>
              </div>
            )}
            <div className="ud-contact-item">
              <MdCalendarToday className="ud-ci-icon" />
              <span>Joined {fmt(user.createdAt)}</span>
            </div>
            <div className="ud-contact-item">
              <MdAccessTime className="ud-ci-icon" />
              <span>Last active: {fmtTime(user.lastActive)}</span>
            </div>
          </div>
        </motion.div>
              ...
              <div className="ud-overview">
                <div className="ud-section-card">
                  ...
                  <div className="ud-info-grid">
                    ...
                    <div className="ud-info-item">
                      <span className="ud-info-label">Email Address</span>
                      <span className="ud-info-value">{user.email}</span>
                    </div>
                    <div className="ud-info-item">
                      <span className="ud-info-label">Phone</span>
                      <span className="ud-info-value">{user.phone || "Not provided"}</span>
                    </div>
                    <div className="ud-info-item">
                      <span className="ud-info-label">Role</span>
                      <span className="ud-info-value" style={{ textTransform: "capitalize" }}>{user.role}</span>
                    </div>
                    <div className="ud-info-item">
                      <span className="ud-info-label">Account Status</span>
                      <span className="ud-info-value" style={{ color: sc.color }}>{user.status}</span>
                    </div>
                    <div className="ud-info-item">
                      <span className="ud-info-label">Member Since</span>
                      <span className="ud-info-value">{fmt(user.createdAt)}</span>
                    </div>
                    <div className="ud-info-item">
                      <span className="ud-info-label">Last Login</span>
                      <span className="ud-info-value">{fmtTime(user.lastActive)}</span>
                    </div>
                    <div className="ud-info-item">
                      <span className="ud-info-label">User ID</span>
                      <span className="ud-info-value ud-mono">{user._id}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="ud-section-card">
                  <div className="ud-section-header">
                    ...
                    <button className="ud-see-all" onClick={() => setActiveTab("orders")}>
                      See all →
                    </button>
                  </div>
                  ...
                  <div className="ud-orders-table">
                    ...
                    {orders.map((o) => (
                      <div key={o._id} className="ud-ot-row">
                        ...
                        <span>
                          <span className={`ud-pay-badge ${o.paymentInfo?.status === "succeeded" ? "paid" : "pending"}`}>
                            {o.paymentInfo?.status === "succeeded" ? "Paid" : "Pending"}
                          </span>
                        </span>
                  ...
                  <div className="ud-address-grid">
                    {user.addresses.map((addr, i) => (
                      <div key={i} className="ud-address-card">
                        ...
                        {addr.phoneNumber && (
                          <p className="ud-addr-phone">
                            <MdPhone /> {addr.phoneNumber}
                          </p>
                        )}
                      </div>
                    ))}
            ...
            {activeTab === "activity" && (
              <div className="ud-activity-wrap">
                    ...
                    {user.lastActive && (
                      <div className="ud-tl-item">
                        ...
                        <div className="ud-tl-body">
                          ...
                          <p className="ud-tl-time">{fmtTime(user.lastActive)}</p>
                        </div>
                      </div>
                    )}
                ...
                <div className="ud-section-card ud-danger-zone">
                  <h4 className="ud-section-title danger">
