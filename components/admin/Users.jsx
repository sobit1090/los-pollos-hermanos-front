...
import { server } from "../../redux/store"; // ✅ adjust path if needed
import { motion } from "framer-motion";
import { 
  MdSearch,
  MdFilterList,
    ...
    <section className="tableClass">
      ...
      <motion.main initial="hidden" animate="visible">
        ...
        <div className="table-container">
          <div className="tableHeader">
            <div>ID</div><div>User Info</div><div>Role</div><div>Status</div><div>Member Since</div><div>Actions</div>
          </div>

          {filteredUsers.map((u, i) => (
            <motion.div key={u._id} className="tableRow">
              <div className="cell">{u._id}</div>

              <div className="cell user-info">
                <img src={u.photo || "https://via.placeholder.com/150?text=User"} alt={u.name} className="userPhoto"/>
                <div className="user-details">
                  <div className="user-name">{u.name}</div>
                  <div className="user-email"><MdEmail /> {u.email}</div>
                </div>
              </div>

              <div className="cell"><span className={getRoleBadgeClass(u.role)}>{u.role}</span></div>
              <div className="cell"><span className={getStatusBadgeClass(u.status)}>{u.status}</span></div>
              <div className="cell"><MdCalendarToday /> {formatDate(u.createdAt)}</div>

              <div className="cell action-buttons">
                <button className="btn-status" onClick={() => handleToggleStatus(u._id, u.status)}>
